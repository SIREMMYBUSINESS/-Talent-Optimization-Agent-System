  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        ...
        {screening && (
          <div
            className={`px-4 py-2 rounded-lg text-center ${getScoreColor(
        ...
        {screening && (
          <div>
            ...
            <div className="space-y-2">
              <div>
                <span
                  className={`px-2 py-1 text-xs font-medium rounded ${getRecommendationColor(
              ...
              {screening.skillsMatch?.matched?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {screening.skillsMatch.matched.slice(0, 5).map(
                      (skill, idx) => (
                        <span
                          key={idx}
              ...
              {screening.skillsMatch?.missing?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {screening.skillsMatch.missing.slice(0, 3).map(
                      (skill, idx) => (
                        <span
                          key={idx}
      ...
      {showModal && (
        <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Candidate Details">
          <p>{candidate.fullName}</p>
          <p>{candidate.email}</p>
          <p>{candidate.experienceYears} years experience</p>
        </Modal>
      )}
    </div>
  );
